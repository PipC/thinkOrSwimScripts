...
def STCTrendChange = if STCTrend >= over_bought + 20 then -1 else 
    if STCTrend <= over_sold - 20 then 1 else 
    if STCTrend < STCTrend[1] then -1 else 1;

plot scan = STCTrendChange[10] < 0 and STCTrendChange > 0 
and STCTrend > STCTrend[1]
and STCTrend <= over_sold
and STCTrend > (over_sold - 5)       
and MovingAverage(AverageType.Simple , STCTrend, 3) < over_sold
