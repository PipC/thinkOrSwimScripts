...
def trending = if STCWave > STCWave[1] then 1 else -1;

AssignBackgroundColor(if STCWave > STCWave[1] then 
    if STCWave < over_bought then createColor(50,180,150) else createColor(50,135,150)
    else 
    if STCWave > over_sold then createColor(180,0,100) else createColor(135,0,100));

plot STCWaveTrending = STCWave * trending;